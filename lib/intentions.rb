...
module Claudine
  ...
  # Decouples event sources from animations: animations are indexed by *intention* (not by hook), a profile maps a source's events onto these intentions, and the temporal `kind` lives here — not hardcoded in the AnimationManager.
  # Adding a source is writing a profile; the render path is untouched.
  module Intentions
    VERSION = "intentions.v1"

    # For each intention:
    #   kind:
    #     :ambient  — background loop, plays as long as the state lasts
    #     :pulse    — one-shot overlay, plays once then reverts to bg
    #     :boundary — start/end that cuts the background (terminal)
    #     :dormant  — idle
    #   fallback: intention to fall back on when a set does not define this one (nil = none; the mandatory-core intentions have no fallback).
    VOCAB = {
    ...
    # Walks the fallback chain until it reaches an intention present in `available` (any object responding to #include? — e.g. the registry keys).
    # Returns that intention, or nil if the chain runs dry.
    # Cycle-safe.
