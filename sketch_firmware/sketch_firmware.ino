...
//   On this FastLED 3.10 / ESP-IDF 5.x pairing, no FastLED backend is reliable for 320 WS2812B on the S3:
...
//   Adafruit NeoPixel relies on the native RMT driver of the Arduino-ESP32 core (the one for the built-in RGB LED), proven on the XIAO S3.
//
// Requires the "Adafruit NeoPixel" library (IDE Library Manager).
//
// Boot animation:
//   A SHORT one-shot sweep on power-up — a dim rainbow filling along the chain, then off — just to show the cube is powered and ready (and that every R/G/B channel responds).
//   It plays ONCE, then the cube stays dark until the Mac app drives it (no standby, no loop).
//   If Adalight frames arrive during the sweep, it yields immediately.
//   Deliberately DIM (BOOT_VAL): the app-driven path runs full scale (the Mac dims), but there's no Mac here, so we cap brightness to stay USB-safe.
...
// NEO_GRB: the WS2812B expect GRB order. The Mac sends R,G,B triplets and handles brightness; we therefore transmit at full scale.
...
void setup() {
  ...
  // the Mac is already sending the next frame. The default USB-CDC RX buffer (256 bytes ~ 85 LEDs) then overflows and corrupts the end of the frame at a variable position.
  // We enlarge it to hold a full frame (6 + 320x3 = 966 bytes).
...
// One-shot "powered & ready" sweep: a dim rainbow filling along the chain, then off.
// Sets bootDone when finished; after that it is never called again.
void renderBoot() {
  uint32_t now = millis();
